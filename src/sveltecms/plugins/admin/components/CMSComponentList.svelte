...
import CmsConfigForm from "sveltecms/plugins/admin/components/CMSConfigForm.svelte";
  ...
  export let data:string[] // An array of all files in the $lib folder
  ...
  let libPath = data.length <= 1 ? (data[0] ?? '').replace(/[^\/]+$/, '') : data.reduce((agg,v) => {
    ...
    for (let i = 0; i < aggArr.length; i++) {
      if (aggArr[i] === vArr[i]) arr.push(aggArr[i])
    }
    return arr.join('/') + '/'
  }, data[0])
  let showLibPath
  ...
  $: if (libPath) data.forEach(f => { if (cms.components[relativePath(f).replace(/\W/g, '_')]) saveData[f] = true })
  ...
  export let save = async () => {
    return fetch('/admin/components', {
      ...
      body: JSON.stringify(Object.fromEntries(
        Object.entries(saveData).map(([filepath,value])=>([relativePath(filepath),value]))
      )),
    })
  }
