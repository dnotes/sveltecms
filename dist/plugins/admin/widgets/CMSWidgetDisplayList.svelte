<script>import { cloneDeep, isEqual, uniq } from "lodash-es";
...
let items = Object.entries(conf).map(([k, v]) => {
...
function getPlaceholders() {
    placeholders = Object.fromEntries(items.map(([mode, val]) => {
        if (typeof value === 'string')
            return [mode, displayConfigToText(value)];
        else if (value?.hasOwnProperty('default'))
            return [mode, displayConfigToText(value['default'])];
        return [mode, fallbacks[mode]];
    }));
}
function displayConfigToText(d) {
    ...
    if (typeof d !== 'string') {
        if (d.wrapper)
            text = displayConfigToText(d.wrapper) + ' > ' + text;
        if (d.label)
            text += ' (label)';
        if (d.html)
            text += ' (html)';
...
function valueShouldBeUndefined(val) {
    if ((typeof val === 'string' && !val) || val?.['type'] === '')
        return true;
    return false;
}
/**
