...
import { ContentType, type ContentTypeConfigSetting } from "sveltecms/core/ContentType";
...
export type ImportFieldConfigSetting = Omit<FieldConfigSetting,'type'|'fields'> & {
  ...
  fields?: {[key:string]:ImportFieldConfigSetting}
}

export type ImportReferenceField = {
  id:string // The ID of the field
  slugConfig:SlugConfig // SlugConfig for making a slug from the value (see the formDataHandler in Widget.ts)
  displayKey:string // The key for the display field
}
  ...
  adminPages: [
    {
      ...
      POST: async ({ cms, args, event, values }) => {
        ...
        // Get passed data
        let data:ImportData = await event.request.json()
        let { contentTypeID:id, slugFields, indexedFields, dir, format, bodyField, createContentType, importContent } = data

        if (!id || id === 'admin' || !id.match(/^[A-Za-z]+$/)) throw new Error(`Invalid Content Type ${id}`)

        let contentType = cms.contentTypes[id]
        let referenceFields:ImportReferenceField[] = contentType
        ...
        let conf:ContentTypeConfigSetting = {
          id,
          slug: slugFields,
          fields: {},
        ...
        filenames.forEach(f => {
          if (!f.match(filenameTest)) return
        ...
        if (createContentType) {
          indexedFields.split(/[\s,]+/).forEach(fieldID => {
            if (get(conf.fields, fieldID)) set(conf.fields, `${fieldID}.index`, true)
          })
        ...
        if (createContentType) cms.conf.contentTypes[id] = conf
...
function getFieldConfFromValue(value:any, conf:void|ImportFieldConfigSetting=undefined):ImportFieldConfigSetting {

  let type = conf ? conf.type : undefined
  // @ts-ignore this works, why is it an error?
  let newConf:ImportFieldConfigSetting = { ...conf, type }
  let v = Array.isArray(value) ? value[0] : value

  let newType:void|ImportFieldTypes = parseValueType(v, type)
  if (!newType) return // ...nothing in this field; no changes, do not save

  newConf.type = newType

  if (newType === 'fieldgroup') {
    if (!newConf.fields) newConf.fields = {}
  ...
  if (Array.isArray(value)) {
    ...
    if (conf && !newConf.multiple) newConf.multipleOrSingle = true
    ...
    if (newConf.multiple) newConf.multipleOrSingle = true
...
function parseValueType(v, type:void|ImportFieldTypes=undefined):void|ImportFieldTypes {

  // For null values, return the existing type
  if (isNull(v) || v === undefined) return type

  // Check if the field is a fieldset
  if (v?.constructor === Object) {
    if (type && (type !== 'fieldgroup')) throw new Error(`non-fieldgroup value in fieldgroup field`)
    return 'fieldgroup'
  }
  else if (type === 'fieldgroup') throw new Error(`fieldgroup value in non-fieldgroup field`)

  // See if the field should be markdown or text
  if (typeof v === 'string') {
    if (v.length > 255) return 'markdown'
  ...
  // Numbers
  if (typeof v === 'number') {
    if (v.toString().includes('.')) return 'float'
    return 'number'
  }
