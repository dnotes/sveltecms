...
export const hooks:PluginHooks = [
  {
    ...
    fn: async (change, cms) => {
      ...
      let referenceFields:[string,string][] = cms.findFields(change.contentType.fields, { type:'reference' })
        .map(fieldPath => {
      ...
      for (let i=0; i<referenceFields.length; i++) {
        ...
        let referencedContentTypes:string[] = Array.isArray(field.widget?.options?.contentTypes)
        ...
        if (!referencedContentTypes.length) referencedContentTypes = cms.listEntities('contentType')

        // Create variable for index changes
        let indexChanges:{[key:string]:IndexChange[]} = {}
        ...
        try {
          ...
          let oldValueArray:IndexItem[] = (Array.isArray(oldValues) ? oldValues : [oldValues].filter(Boolean)).filter(isIndexItem)

          let newValues = get(change.after || {}, fieldPath) ?? []
          let newValueArray:IndexItem[] = (Array.isArray(newValues) ? newValues : [newValues].filter(Boolean)).filter(isIndexItem)
              ...
              if (before) {
                ...
                if (!Array.isArray(fieldValue)) { // could be a !multiple reference field
                  if (fieldValue?.['_type'] === oldIndexItem._type && fieldValue?.['_slug'] === oldIndexItem._slug) {
                    ...
                    if (allContent[_type]) allContent[_type][_slug] = after
                    else if (indexChanges[_type]) indexChanges[_type].push({ before, after })
                  ...
                  if (idx !== -1) {
                    ...
                    let contentType:ContentType|undefined = cms.contentTypes[_type]
                    let referenceField:Field|undefined = get(contentType?.fields || {}, referenceKey)

                    // Set the value in the other Content|IndexItem
                    fieldValue.splice(idx,1)
                    if (!fieldValue.length) set(after, referenceKey, undefined)
                    else if (fieldValue.length === 1 && referenceField && referenceField.multipleOrSingle) set (after, referenceKey, fieldValue[0])
                    else set(after, referenceKey, fieldValue)
          ...
          let updateRefs = newValueArray
            .map(async item => {
              ...
              if (!referencedContentTypes.includes(_type)) return
              ...
              if (!isChangedContentIndexItem && findReferenceIndex(item, oldValueArray) !== -1) return
              ...
              if (!after) {
                if (referencedContentTypes.length !== 1 || !field.widget?.options?.freeTagging) return
              ...
              // If it's not an array, add the item if allowed. Items get added at the top.
                ...
                if (!referenceField || (referenceField?.multiple && referenceField?.multipleMax > 1)) set(after, referenceKey, [newIndexItem, fieldValue])
              }
              // Add the item
                ...
                let idx = findReferenceIndex(oldIndexItem, fieldValue)
                if (idx !== -1) fieldValue.splice(idx, 1, newIndexItem)
                else fieldValue.unshift(newIndexItem)
          ...
          promises = Object.keys(indexChanges).map(async id => {
            if (indexChanges[id].length) await cms.indexer.updateIndex(id, indexChanges[id])
          })
      ...
      if (errors.length) throw new Error(errors.map(e => `Reference linking: ${e.message}\n${e.stack||''}\n\n`).join('\n'))
