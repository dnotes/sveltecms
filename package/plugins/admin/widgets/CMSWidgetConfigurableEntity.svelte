...
export let items; // The list of items for the select box, or a single entity ID for default items
let exportedValue; // The config setting exported as value
export { exportedValue as value };
export let disabled = undefined; // Whether the select box should be disabled
export let unset = undefined; // The label for the unset value
export let forceEntityID = undefined; // A single entity ID to force configuration, e.g. for default items
...
function saveOptions() {
    ...
    if (isEqual(Object.keys(newValue), ['type']))
        newValue = forceEntityID ? '' : newValue?.['type'];
