<script context="module" lang="ts">
  // from https://github.com/muonw/powertable
  // using a github repository messes up StackBlitz
        ...
        Object.keys(row).forEach(key => {
            ...
            if (!specialInstructs.hasOwnProperty(key)) {
                if (row[key] === null) {
                  ...
                  if (!entity?.fields?.[key]?.displayComponent) row[key] = JSON.stringify(row[key]);
                } else {
...
<div class="powertable" data-name="main-container" data-prefix="{options.uniquePrefix}" data-status={renderStatus}>
                                            ...
                                            {#each instructs as instruct}
                                                ...
                                                {:else}
                                                    <td data-key={instruct.key}>
                                                        ...
                                                        {:else}
                                                            {#if entity?.fields?.[instruct.key]?.displayComponent}
                                                              {#await entity.fields[instruct.key].displayComponent?.component then component}
                                                                <svelte:component this={component} {cms} item={record[instruct.key]} entity={get(entity.fields, instruct.key)} />
                                                              {/await}
                                                            {:else}
                                                              {(record[instruct.key] ?? '')}
                                                            {/if}
...
<style>
  ...
  div.powertable table {
    width:100%;
  }
  div.powertable :global(td) {
    cursor: pointer;
    padding: 5px 8px;
  }
  div.powertable :global(button) {
    border: none;
  }
  div.powertable :global(th[data-dir="asc"]:after) {
    content: "🔼";
    font-weight: normal;
    opacity: .6;
  }
  div.powertable :global(th[data-dir="desc"]:after) {
    content: "🔽";
    font-weight: normal;
    opacity: .6;
  }
  div.powertable :global([data-key^="_"]) { display:none; }
  div.powertable :global(td img) {
    max-width: 80px;
    max-height: 80px;
    display: inline-block;
  }
  div.powertable :global(td img:after) {
    content: attr(alt);
  }

  div.powertable :global(div[data-name="topBar"]),
  div.powertable :global(div[data-name="bottomBar"]){
    display:flex;
    gap: 20px;
    flex-wrap: wrap;
    border-top: 2px solid var(--cms-main);
    border-bottom: 2px solid var(--cms-main);
    padding: 4px 0;
  }

  div.powertable :global(div[data-name="topBar"] div),
  div.powertable :global(div[data-name="bottomBar"] div){
    flex-grow: 1;
  }

  div.powertable :global(div[data-name="search-container"] label span),
  div.powertable :global(div[data-name="dropdown-container"]),
  div.powertable :global(div[data-name="stats-container"]),
  div.powertable :global(div[data-name="pagination-container"] button) {
  ...
  div.powertable :global(div[data-name="pagination-container"]) {
    text-align: right;
  }

  div.powertable :global(thead tr) {
