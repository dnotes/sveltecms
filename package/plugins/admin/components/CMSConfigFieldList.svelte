...
function getFieldFormID(id) { return opts?.id ? `${opts.id}[${id}]` : id; }
...
function shiftItem(i, up) {
    if (i === 0 && up || i === items.length - 1 && !up)
        return;
    let entry = items.splice(i, 1)[0];
    let newIndex = up ? i - 1 : i + 1;
    if (newIndex)
        items = [...items.splice(0, newIndex), entry, ...items];
    else
        items = [entry, ...items];
    items = items;
}
...
async function setProp(i, prop, value) {
    ...
    if (prop === 'type') {
        ...
        if (typeof value === 'undefined' || value === (d.parentField[prop] ?? d.fieldType[prop] ?? false))
            delete e[1][prop];
        else
            e[1][prop] = value;
    }
    if (isEqual(e[1], { type: e[1].type }))
        e[1] = e[1].type;
    items = items;
}
