...
import { cloneDeep } from "lodash-es";
  ...
  async function addItem() {
    ...
    if (value.length === 0 && field?.multipleOrSingle) value = cloneDeep(field.default)
    else value = [...value, cloneDeep(field.default)]
    await tick()
  ...
  async function removeItem(i) {
    if (!Array.isArray(value)) value = []
      ...
      value.splice(i,1)
      if (value.length === 1 && field?.multipleOrSingle) value = value[0]
      else value = value
    }
  }
...
<style global>
  .sveltecms .multiple-item { position:relative; }
  .sveltecms .multiple-item>.delete { position:absolute; top:.25em; right:.5em; }
</style>